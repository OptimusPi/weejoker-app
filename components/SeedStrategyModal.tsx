...
export function SeedStrategyModal({ analysis, onClose }: SeedStrategyModalProps) {
    ...
    return createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-[var(--jimbo-panel-edge)] animate-in fade-in duration-200" onClick={onClose}>
            <JimboPanel className="w-full max-w-4xl h-[90vh] flex flex-col relative !p-0 overflow-hidden" onClick={e => e.stopPropagation()}>

                {/* Header */}
                <div className="shrink-0 p-4 border-b border-transparent flex justify-between items-center bg-[#1a1a1a]">
                    <div>
                        <div className="text-[var(--jimbo-blue)] font-header text-2xl tracking-widest drop-shadow-md">
                            SEED STRATEGY
                        </div>
                        <div className="text-[var(--jimbo-grey)] font-pixel text-xs mt-1">
                    ...
                    <button
                        ...
                        title="Close"
                        aria-label="Close"
                        className="jimbo-btn jimbo-btn-red !py-1 !px-3 font-header text-xl"
                    >
                ...
                <div className="flex flex-1 overflow-hidden">
                        ...
                        {Array.from({ length: maxAnte }).map((_, i) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="truncate text-sm opacity-100 text-white">{boss || "???"}</div>
                                </button>
                            );
                        })}
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-6 bg-transparent">
                        {anteData ? (
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-2 flex-wrap">
                                        {anteData.tags.map((tag, i) => (
                                            <div key={i} className="px-3 py-1 bg-[#441] border border-[var(--jimbo-gold)] rounded text-[var(--jimbo-gold)] font-pixel text-xs">
                                                {tag}
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    <JimboInnerPanel className="p-3 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                                        {anteData.shopQueue.map((item, i) => (
                                            <div key={i} className={cn(
                                                ...
                                                <div className="font-pixel text-[10px] text-[var(--jimbo-border-silver)] truncate">{item.name}</div>
                                                {item.edition && (
                                                    <div className={cn(
                                    ...
                                    <div className="space-y-3">
                                        {anteData.packs.map((pack, i) => (
                                            <JimboInnerPanel key={i} className="p-3 bg-[#111]">
                                                <div className="font-header text-lg text-[var(--jimbo-blue)] mb-2">{pack.name}</div>
                                                <div className="flex gap-2 overflow-x-auto pb-1 custom-scrollbar">
                                                    {pack.cards.map((card, ci) => (
                                                        <div key={ci} className="shrink-0 w-24 h-32 bg-[#222] border border-transparent rounded flex items-center justify-center p-2 text-center text-xs font-pixel text-[#eee] transition-colors">
                                                            {card.name}
                                                            {card.edition && (
                                                                <div className="block mt-1 text-[8px] text-[var(--jimbo-gold)]">{card.edition}</div>
                                                            )}
                                                        </div>
                                                    ))}
                                                </div>
                                        ...
                                        {anteData.packs.length === 0 && (
                                            <div className="text-[var(--jimbo-grey)] font-pixel text-xs italic">No packs opened this ante.</div>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div className="h-full flex items-center justify-center text-[var(--jimbo-grey)] font-header text-2xl">
